...
export type Activity = {
  ...
  rating: number;
  // Equally editorial, and equally never shown on its own — the card prints it
  // next to `rating` ("4.3 · 2,847 reviews"), so it is gated by the same flag.
  reviewCount: number;
  ...
  // vocabulary with live Viator items. Without these a local snorkel beach and
  // a paid snorkel catamaran are invisible to each other: every semantic dedup
  // signal (cluster id, tag Jaccard) is Viator-only, so nothing could compare
  // them and both landed on the same day. Ids come from KIND_BY_TAG in
  // itemFit.ts — 11912 snorkel, 11888/11885 sail+day-cruise, 12021 dive,
  // 12047 kayak, 13202 surf, 11902 hike, 12035 off-road.
  tags?: number[];
  matched_by: MatchTag[];
  ...
  bookingUrl?: string;
  /**
   * A restaurant's reservation channel — distinct from `bookingUrl` for the
   * same reason that field is distinct from `viator_item_url`: a table
   * reservation is not a paid booking, and the card words it differently
   * ("Reserve a table" / "Call to reserve" vs "Book now"). `kind` feeds the
   * `restaurant_reserve_click` analytics event, whose per-restaurant counts
   * are the sales collateral for stage 1 of
   * docs/strategy/2026-08-24-restaurant-monetization/. `opentable` is for
   * restaurants listed there (none curated yet); `phone` uses a tel: URI.
   */
  reserve?: { url: string; kind: 'opentable' | 'web' | 'phone' };
...
export const ACTIVITIES: Activity[] = [
  ...
  { id: 'kitesurfing-lesson', title: "Kitesurfing at Fisherman's Huts", category: 'Watersports', image: '/Kitesurfing Fishermans Huts.webp', description: "Constant 15–25 knot trade winds, shallow flat water. Aruba is one of the world's top kite spots.", localsSay: '"Consistent wind 320+ days a year." — Sven', cost: '$120 lesson', duration: '2–3 hrs', timeOfDay: 'Afternoon', fitReason: 'World-class kite conditions', location: "Fisherman's Huts, Malmok", rating: 4.8, reviewCount: 654, adventure: 85, sections: ['cruises-water'], tags: [13202], matched_by: [] },
  { id: 'natural-pool-jeep', title: 'Natural Pool 4x4 & Snorkel Tour', category: 'Tours', image: '/Natural Pool 4x4 Tour.webp', description: "Rugged 4x4 drive across Arikok's volcanic dunes to the lava-ringed Conchi natural pool — then snorkel in its sheltered cove.", localsSay: '"Not easy to get to. That\'s the whole point." — Marco', cost: '$75 pp', duration: '3–5 hrs', timeOfDay: 'Morning', fitReason: 'Off-the-beaten-path adventure', location: 'Arikok', rating: 4.9, reviewCount: 1102, adventure: 70, sections: ['adventures-outdoor'], tags: [12035, 11912], matched_by: [] },
  { id: 'malmok-beach', title: 'Malmok Beach Snorkel', category: 'Beaches', image: '/Malmok Beach Snorkel.webp', description: "Calm, shallow water over a rocky bottom — Aruba's best shore-snorkel beach. Green turtles and reef fish right off the rocks, with the Antilla wreck just offshore.", localsSay: '"Get in before the catamarans arrive around 10 and the turtles are all yours." — Edsel', cost: 'Free + $16 gear', duration: '2–3 hrs', timeOfDay: 'Morning', fitReason: 'Best shore snorkeling', location: 'Malmok, Noord', rating: 4.7, reviewCount: 1118, adventure: 28, sections: ['beaches'], tags: [11912], matched_by: [] },
  { id: 'tres-trapi', title: 'Tres Trapi Turtle Cove', category: 'Beaches', image: 'https://images.pexels.com/photos/1645028/pexels-photo-1645028.jpeg?auto=compress&cs=tinysrgb&w=800', description: '"Three steps" — concrete stairs drop you into a sheltered cove where green turtles graze the seagrass. Free shore entry, no boat needed, just south of Boca Catalina.', localsSay: '"Walk down the steps slowly, float, and wait. They surface for air right beside you." — Glennis', cost: 'Free + $16 gear', duration: '2–3 hrs', timeOfDay: 'Morning', fitReason: 'Free turtle snorkeling', location: 'Tres Trapi, Noord', rating: 4.8, reviewCount: 1452, adventure: 25, sections: ['beaches'], tags: [11912], matched_by: [] },
  { id: 'manchebo-beach', title: 'Manchebo Beach Sunset', category: 'Beaches', image: '/Manchebo Beach Sunset.webp', description: "Aruba's widest stretch of sand, far quieter than neighbouring Eagle. Acres of space, a handful of thatched palapas, and one of the island's best unobstructed sunsets.", localsSay: '"Eagle gets the crowds — walk five minutes south to Manchebo and it\'s just you and the sunset." — Yarzagaray', cost: 'Free', duration: '2–3 hrs', timeOfDay: 'Evening', fitReason: 'Widest sand, quiet sunset', location: 'Manchebo, Eagle Beach', rating: 4.7, reviewCount: 982, adventure: 6, sections: ['beaches'], matched_by: [] },
  ...
  { id: 'mangel-halto', title: 'Mangel Halto Lagoon', requires_car: true, category: 'Beaches', image: '/Mangel Halto Lagoon.webp', description: 'A mangrove-fringed lagoon on the calm south coast. Wade in from the dock or kayak the channels; just past the mangroves the reef drops into a wall of fish.', localsSay: '"Snorkel out through the cut in the reef — but only on a calm day, and always head back before the current turns." — Orlando', cost: 'Free + $16 gear', duration: '2–4 hrs', timeOfDay: 'Morning', fitReason: 'Mangrove lagoon snorkel', location: 'Pos Chiquito, Savaneta', rating: 4.7, reviewCount: 874, adventure: 25, sections: ['beaches'], matched_by: [] },
  { id: 'rodgers-beach', title: "Rodger's Beach", requires_car: true, category: 'Beaches', image: '/Rodgers Beach.webp', description: 'A calm crescent right next to Baby Beach, framed by the old refinery hills. Glassy water, a local snack truck, and a fraction of the crowds.', localsSay: '"Everyone piles into Baby Beach. Rodger\'s, right next door, is where San Nicolas actually swims." — Ditmar', cost: 'Free', duration: '2–3 hrs', timeOfDay: 'Afternoon', fitReason: 'Calm, uncrowded south', location: 'Seroe Colorado, San Nicolas', rating: 4.6, reviewCount: 717, adventure: 8, sections: ['beaches'], matched_by: [] },
  { id: 'boca-grandi', title: 'Boca Grandi', requires_car: true, category: 'Beaches', image: '/Boca Grandi.webp', description: 'Wild and windswept on the southeast coast — powerful surf and steady cross-shore wind make this a kitesurf and bodyboard beach, not a swimming one. Come for the drama, not the dip.', localsSay: '"Not for swimming — the current is no joke. But for wind, waves and an empty beach, nowhere beats it." — Shendrick', cost: 'Free', duration: '1–2 hrs', timeOfDay: 'Afternoon', fitReason: 'Wild surf & wind', location: 'Boca Grandi, San Nicolas', rating: 4.6, reviewCount: 543, adventure: 30, sections: ['beaches'], matched_by: [] },
  // ── Added for the curated "Balanced" template ───────────────────────────
  // Photos are NOT yet in public/ — see the list handed back with this change.
  // `localsSay` is deliberately empty: it is data-only (nothing renders it) and
  // inventing quotes attributed to named locals for a live site is not on.
  // No coordinates: the project decided against rough pins (2a7b7c8), so these
  // go through the pin-review workflow rather than being guessed here.
  { id: 'arashi-beach', title: 'Arashi Beach', requires_car: true, category: 'Beaches', image: '/Arashi Beach.webp', description: "The last beach before the island's northwest tip — calm, shallow and sandy, with a reef close enough to swim to. Palapas, a snack bar, and none of the high-rise crowd.", localsSay: '', cost: 'Free', duration: '2–3 hrs', timeOfDay: 'Morning', fitReason: 'Calm snorkel beach, no crowds', location: 'Arashi, Noord', rating: 4.7, reviewCount: 0, adventure: 12, sections: ['beaches'], tags: [11912], matched_by: [] },
  { id: 'palm-beach-strip', title: 'Palm Beach', category: 'Beaches', image: '/Palm Beach afternoon.webp', description: 'The wide, hotel-lined stretch north of Eagle — flat water, watersports desks, and the pier bars. Busiest beach on the island, and the easiest one to spend a whole afternoon on.', localsSay: '', cost: 'Free', duration: '2–4 hrs', timeOfDay: 'Afternoon', fitReason: 'Everything within walking distance', location: 'Palm Beach, Noord', rating: 4.6, reviewCount: 0, adventure: 6, sections: ['beaches'], matched_by: [] },
  { id: 'boca-catalina-shore', title: 'Boca Catalina Shore Snorkel', requires_car: true, category: 'Beaches', image: '/Boca Catalina Beach.webp', description: 'A small rocky cove between Malmok and Arashi where the catamarans anchor offshore. Walk in from the road, swim out twenty metres, and the fish are already there — no boat needed.', localsSay: '', cost: 'Free + $16 gear', duration: '1–3 hrs', timeOfDay: 'Morning', fitReason: 'Boat-free reef access', location: 'Boca Catalina, Malmok', rating: 4.7, reviewCount: 0, adventure: 22, sections: ['beaches'], tags: [11912], matched_by: [] },
  { id: 'alto-vista-chapel', title: 'Alto Vista Chapel & Hilltop Walk', requires_car: true, category: 'Activities', image: '/Alto Vista Chapel.webp', description: "Aruba's first chapel, alone on a hill above the north coast, reached by a lane lined with white crosses. Short walk, big view, and it empties out completely in the late afternoon.", localsSay: '', cost: 'Free', duration: '1–2 hrs', timeOfDay: 'Afternoon', fitReason: 'Quiet history, golden hour', location: 'Alto Vista, Noord', rating: 4.6, reviewCount: 0, adventure: 20, sections: ['culture-history'], tags: [11902], matched_by: [] },
  { id: 'california-dunes-sunset', title: 'California Dunes at Sunset', requires_car: true, category: 'Activities', image: '/Lighthouse Dunes Sunset.jpeg', description: 'Rolling white sand dunes behind the lighthouse, walkable in an hour. Go late: the low sun throws the ridges into relief and the whole northwest coast is below you.', localsSay: '', cost: 'Free', duration: '1–1.5 hrs', timeOfDay: 'Evening', fitReason: 'Best light on the island', location: 'Hudishibana, Noord', rating: 4.5, reviewCount: 0, adventure: 25, sections: ['adventures-outdoor'], tags: [11902], matched_by: [] },
  { id: 'bushiribana-loop', title: 'Bushiribana Coastal Walk', requires_car: true, category: 'Activities', image: '/Bushiribana ruins.webp', description: 'The shell of a 19th-century gold smelter on the windward coast, with a rough loop along the cliffs from Calbas. Exposed, windy, and the sea here does not stop.', localsSay: '', cost: 'Free', duration: '2–3 hrs', timeOfDay: 'Morning', fitReason: 'Ruins and a wild coastline', location: 'Bushiribana, north coast', rating: 4.4, reviewCount: 0, adventure: 50, sections: ['adventures-outdoor'], tags: [11902], matched_by: [] },
  { id: 'san-nicolas-murals', title: 'San Nicolas Mural Walk', requires_car: true, category: 'Tours', image: '/San Nicolaas Mural.webp', description: "The old refinery town turned open-air gallery — dozens of large-scale murals across a few walkable blocks, plus Aruba's oldest bars. Self-guided, or join a walking tour.", localsSay: '', cost: 'Free', duration: '2–3 hrs', timeOfDay: 'Morning', fitReason: 'Street art and real town life', location: 'San Nicolas', rating: 4.6, reviewCount: 0, adventure: 10, sections: ['culture-history'], matched_by: [] },
];
