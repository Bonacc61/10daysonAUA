...
| 10 | **The engine still slots by title, though it no longer has to** | The data landed 2026-08-13 (`8b3f222`) and is committed at `src/data/startTimes.json`, but only the CARDS read it — `itemSlotOk`/`itemSlotOkForFill` are unchanged, so placement is still title-only and 62666P1 can still be planned for an afternoon while its card now says "Departs 9:00am". Measured impact of wiring it: **114** products currently have no title signal and would gain a real constraint, and **0** titles are contradicted by their schedule, so this can only tighten placement, never overturn it. Two constraints on how: it belongs in `itemSlotOk` as a CORRECTNESS claim like the Arikok gate, never a preference (a preference there re-faces saved and shared itineraries — see the comment on `itemSlotOkForFill`); and only the **162** morning-only/afternoon-only products qualify, because the **119** that straddle noon constrain nothing. Expect generated plans to move: measure with `npm run plan-diff` before and after. |
