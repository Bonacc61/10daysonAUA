...
| # | Item | Notes |
|---|---|---|
| 1 | ~~`feedback_events` retention cron~~ — **applied 2026-08-13**, but a consent question is open | `20260813140000_feedback_retention_cron.sql` schedules a nightly purge at 24 months, mirroring the contact-form job. Nothing is deleted yet: the oldest row is 2026-05-23. Reverse with `cron.unschedule('purge-old-feedback-events')`. **The bigger find was that this collection was undocumented entirely** — the Privacy Policy's "Data we collect" table had no row for it, while `feedback_events` has recorded every swap/add/remove/move against a browser id since May. A row is added now under legitimate interest. **Still open, and it is a legal call not a technical one:** `logEvent` fires UNCONDITIONALLY — `src/data/feedback.ts` has no consent check, unlike `analytics.ts` which gates PostHog on `10doa:analytics-consent`. The data is pseudonymous and carries no typed words (the "why swap?" is a chip id or the literal `nl`), so legitimate interest is arguable — but `aruba.session` is a non-essential identifier stored on the device, which under ePrivacy points at consent rather than legitimate interest. Either gate it behind the existing banner or take advice; do not leave it undecided.
