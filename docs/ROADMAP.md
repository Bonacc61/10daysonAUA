...
| 8 | **Account deletion — built and deployed, but the destructive path is UNTESTED and the button is off** | `supabase/functions/account-delete` is live. It reads the uid from the TOKEN (never the body, so a caller can only delete themselves), deletes `shared_itineraries` explicitly, then the auth user — which cascades `trips`. Guards verified against the deployed function: GET→405, no-auth→401, anon key as bearer→401, garbage token→401, OPTIONS→200. **The delete itself has never been executed.** A disposable account cannot reach it: signup requires email confirmation, so no session token is issued (attempted 2026-08-13; it left an unconfirmed `delete-test-1786663469@10daysonaruba.com` in `auth.users` that needs removing from the dashboard). **The UI is therefore hidden** — `accountDeletionAvailable()` returns false while `VITE_ACCOUNT_DELETE_FN_URL` is absent from `.env.production`, which it deliberately is. To enable: confirm a real throwaway account, run the flow once, watch the row disappear, THEN add the var. Two schema facts this fixed, worth keeping: `shared_itineraries` has no delete policy at all (immutable by design) and its FK is `on delete set null`, so without the explicit delete the account would vanish and leave a live public `/i/<slug>` behind — while the Privacy Policy promises snapshots last only "until you delete it or your account". `feedback_events` is NOT covered and cannot be: it is keyed by a browser `session_id` with no user column, which is GDPR Art. 11 — linking identity to it purely to enable erasure would be worse for the traveller. It is consent-gated and purges at 24 months. |
