...
| 9 | **Boat check-in times are 9 cards deep, not 135** | Shipped 2026-08-12 on itinerary cards; **Explore's grid cards render it too since 2026-08-13** — both surfaces now go through `src/components/DepartureNote.tsx`, so the rule that the site never states a departure time in its own voice lives in one file rather than two copies. There is no component-test infrastructure in this repo (no testing-library, no jsdom), so nothing guards the MARKUP; `departurePointFor` is well covered and the extraction is what stops the two surfaces drifting. Water-based products show "Departs from &lt;place&gt;" from the committed pin registry (**35 of 135** water items qualify; **11** of those are `approx` pins and say "near" instead), and quote the operator's own check-in sentence where one was captured (**9**). There is no automatic source for the rest — measured 2026-08-12, exactly 1 of 135 descriptions states a clock time, and the 2026-08-03 Viator probe found meeting-point data unusable (`docs/map/viator-location-probe.md`). Widening it means hand-research per product, the same way coordinates were done: add a verbatim line to `CHECKIN_QUOTES` in `src/data/itemCoords.ts` and it renders with no code change. **Never paraphrase into a synthesised time** — a test enforces that every quote is operator wording. |

| 10 | **The engine slots 81% of the catalog with no idea when it runs** | Measured 2026-08-13 on the live catalog (366 items): time-of-day for a Viator product comes from the TITLE alone — 14 titles name morning/afternoon (`titleTimeOfDay`), 57 match the evening regex, plus the Arikok/Natural Pool special case. **295 of 366 carry no time signal at all** and are placed morning-or-afternoon arbitrarily. That is why "Aruba Downtown Historic and Cultural Walking Tour" (`62666P1`, a 09:00 departure on Viator) is suggested for an afternoon. Descriptions cannot fix it: **3 of 366** contain any clock time, and **348 of 366 are truncated** at 200 chars by `shortDescription` in `normalize.ts`, so the source text is cut off before any schedule line. Fix is INGEST, not a rule — see item 11. |
| 11 | **Ingest Viator start times** | `/availability/schedules/{product-code}` returns `bookableItems[].seasons[].pricingRecords[].timedEntries[].startTime` — the authoritative source, and Viator's docs put single-product availability inside **Basic Access**, the tier we already use (`/products/search` + `/products/{code}`). So this needs **no tier upgrade, but one call per product** — ~366 calls at ingest, which is a build-time cost, not a per-visitor one. `/availability/schedules/bulk` does 500 products in ONE call but sits behind **Full Access** (pre-approval + certification); worth applying for separately, not a blocker. **Probe before building**, the way the 2026-08-03 meeting-point probe was run: measure how many of the 366 actually return timed entries, because open-ended admission tickets legitimately have none. Then note the modelling trap — `startTime` is per season AND per day-of-week, so a product has a SET of start times, not one. The only safe rule is "if EVERY start time falls before ~12:00 it is a morning product", stated in `itemSlotOk` as a correctness claim like the Arikok gate, never a preference (a preference there re-faces saved and shared itineraries). Rendering it on the card is a SEPARATE decision: `CHECKIN_QUOTES` renders operator *words*, and a test enforces that; a start time is structured data, so printing it is the site asserting a fact in its own voice. |
