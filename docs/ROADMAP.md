...
| 7a | Template pre-pass bypasses the day-pass rule | `templateAvail` checks whether the SLOT is claimed, never the day, and `fitsDayShape` is declared after the template block (TDZ — it cannot be called there even if someone tried). Dormant only because nothing passes `opts.pinned`; with the pin path re-enabled a review measured **64 of 100 runs** putting a second card on the pass's day. **Close this before rewiring the shortlist to `opts.pinned`** or the day-pass bug reopens. Found 2026-08-12 in review. |
| 7b | Day-pass "reverse direction" is untested | `is not placed on a day that already has something` never constructs a day that already has something, so it dies to the same mutation as the test above it. Both `fitsDayShape` gates can be deleted with the full suite still green. Give it a real prior-occupancy day (a pin or a staple) or drop it. Found 2026-08-12 in review. |
| 7c | `plan-diff` can report a false pass | `arg()` uses `parseInt` with no `Number.isFinite` guard, so `--days abc` yields `NaN days`, near-empty plans, "0 violations" and **exit 0**. And with an empty snapshot `stripEnrichment` returns the same catalog, so the tool compares 20 plans against themselves — it says so, but still exits 0. The absolute violation count is real; the *diff* is not. Fix both before trusting the exit code as a CI gate. Found 2026-08-12 in review. |
