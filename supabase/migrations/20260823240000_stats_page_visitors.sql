-- Per-page unique visitors, alongside the pageview count.
--
-- WHY: "what percentage of visitors reached Explore" could not be read off this
-- page. The Pages list counted OPENS, so a visitor who opened Explore three
-- times was three, and there was no denominator to divide by anyway. Answering
-- it meant querying the table by hand.
--
-- Ordered by VISITORS now, not opens: the list is read as "where did people go",
-- and a page opened many times by few people should not outrank one that most
-- visitors saw once.
--
-- Over a window longer than a day these are visitor-days, exactly as everywhere
-- else on this page — the code rotates at midnight UTC — and the page labels
-- them the same way.
