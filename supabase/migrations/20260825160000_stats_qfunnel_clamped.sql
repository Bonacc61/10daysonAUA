-- Questionnaire drop-off, second cut — the first night of real data showed
-- two ways the card misled (owner report, 2026-08-25 evening):
--
--   1. "Generated an itinerary: 20 unique, 133%". itinerary_generated is not
--      a subset of questionnaire openers — returners with saved answers go
--      straight to the itinerary — so the row is dropped from this key; the
--      client now fires q_reached_8 from the Build button instead.
--   2. The per-question rows were born at 22:40 that evening while viewed /
--      started reach back to the 23rd, so any wider window divided different
--      eras. The key's window is now clamped to the birthdate.
--
-- Everything outside the questionnaireFunnel key is copied unchanged from
-- 20260825150000.
...
select jsonb_build_object(
  ...
  -- Where the questionnaire loses people. Distinct visitors per question
  -- REACHED (q_reached_N fires on arriving at question N; q_reached_8 is the
  -- Build button on the last question, not an eighth question).
  --
  -- CLAMPED to the instrumentation birthdate, unlike every other key: a window
  -- reaching further back mixes eras — openers counted for days against
  -- question rows counted for hours — and the card read as "everyone bounces
  -- at Q2". itinerary_generated is deliberately NOT in this object any more:
  -- returners with saved answers regenerate without opening the questionnaire,
  -- which put that row at >100% of openers.
  --
  -- Same daily-hash caveat as everywhere else: across a multi-day window these
  -- are visitor-days, and a traveller finishing after midnight UTC is two.
  'questionnaireFunnel', (
    with qwin as (
